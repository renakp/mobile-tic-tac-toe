...
public class MainMIDlet extends MIDlet {
	...
	private final Command startCommand = new Command("Start", "Start game",
			Command.SCREEN, 1);
	private final Command endCommand = new Command("End", "End game",
			Command.BACK, 1);
		...
		welcomeScreen.setCommandListener(new CommandListener() {
			public void commandAction(Command c, Displayable d) {
				if (c == exitCommand) {
					...
					if (c == startCommand) display.setCurrent(gameScreen);
			}
		});
	}

	private void createGameScreen() {
		gameScreen = new GameScreen(display);
		...
		gameScreen.setCommandListener(new CommandListener() {
			public void commandAction(Command c, Displayable d) {
				if (c == exitCommand) {
					...
					if (c == endCommand) display.setCurrent(welcomeScreen);
			}
		});
	}
